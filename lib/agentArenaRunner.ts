...
declare global {
  var __miraixArenaRunner__: RunnerState | undefined;
}

function getRunnerState(): RunnerState {
  if (!globalThis.__miraixArenaRunner__) {
    globalThis.__miraixArenaRunner__ = {
      started: false,
      timer: null,
      running: false,
    };
  ...
  return globalThis.__miraixArenaRunner__;
}
...
function computeSignal(
  ...
  if (direction === "short") {
    return {
      enterLong: false,
      exitLong: true,
      note: "Short-only strategies are not enabled on the spot demo runner yet.",
    };
  }
