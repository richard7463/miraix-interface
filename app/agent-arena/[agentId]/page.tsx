...
const copy = {
  en: {
    ...
    sources: {
      marketHistoryLive: "Live OKX public candle history",
      marketHistoryFallback: "Fallback curve because live OKX history is unavailable",
      portfolioLive: "Live OKX portfolio snapshot",
      portfolioFallback: "Demo snapshot until OKX private credentials are configured",
      positionsLive: "Positions loaded from OKX private portfolio",
      positionsFallback: "Sample positions shown because live private portfolio is not configured",
    },
    modes: {
      running: "Running",
    },
  ...
  zh: {
    ...
    sources: {
      marketHistoryLive: "实时 OKX 公共 K 线历史",
      marketHistoryFallback: "当前未取到实时 OKX K 线，回退为示例曲线",
      portfolioLive: "实时 OKX 账户快照",
      portfolioFallback: "当前未配置 OKX 私有凭据，显示示例账户快照",
      positionsLive: "持仓来自 OKX 私有账户",
      positionsFallback: "当前持仓为示例数据，未连接真实私有账户",
    },
    modes: {
      running: "运行中",
    },
...
function summarizeOperationalNote(note: string | null | undefined, locale: ArenaLocale) {
  ...
  if (note.includes("Parameter clOrdId error")) {
    return locale === "zh"
      ? "OKX 拒绝了本次委托，原因是 clOrdId 参数无效。runner 会在下一轮继续处理。"
      : "OKX rejected the order because the clOrdId parameter was invalid. The runner will continue on the next cycle.";
  }
...
export default function AgentArenaDetailPage() {
  ...
  const [copyAlias, setCopyAlias] = useState("Demo follower");
  ...
  async function loadAgent(targetAgentId: string, showLoader = true) {
    ...
    try {
      const response = await fetch(`/api/agent-arena/${targetAgentId}`, {
        cache: "no-store",
      });
  ...
  async function toggleFollow() {
    ...
    try {
      ...
      const response = await fetch(`/api/agent-arena/${agentId}/follow`, {
  ...
  async function startDemoCopyTrade() {
    ...
    try {
      ...
      const response = await fetch(`/api/agent-arena/${agentId}/copy`, {
        method: "POST",
  ...
  async function stopDemoCopyTrade(followerId: string) {
    ...
    try {
      setIsCopyPending(true);
      const response = await fetch(`/api/agent-arena/${agentId}/copy?followerId=${encodeURIComponent(followerId)}`, {
        method: "DELETE",
      });
      const payload = await response.json().catch(() => null);
      if (!response.ok || !payload?.ok) {
  ...
  const portfolioSourceLabel =
    hasRuntimeLedger && runtime
      ? locale === "zh"
        ? "Runner 持续汇总的真实 demo 账户结果"
        : "Real demo account results aggregated by the runner"
      : !agent?.localOnly
        ? locale === "zh"
          ? "官方策略档案快照，不绑定独立 OKX 私有账户"
          : "Official strategy profile snapshot without a dedicated OKX private account"
  ...
  const executionNote =
    hasRuntimeLedger && runtime
      ? runtime.totalOrders > 0
          ? locale === "zh"
            ? "执行证据来自 runner 持续同步的 OKX demo 订单与成交。"
            : "Execution evidence is coming from the runner-synced OKX demo orders and fills."
          : locale === "zh"
            ? "runner 已注册，但当前还没有真实订单。"
            : "The runner is registered, but there are no real orders yet."
      : !agent?.localOnly
        ? locale === "zh"
          ? "这个官方代理当前没有独立的 OKX 账户或 runner。执行证据只对已提交并绑定自身运行时的代理显示。"
          : "This official agent does not have a dedicated OKX account or runner yet. Execution evidence is only shown for submitted agents with their own runtime."
      : execution?.note ??
        (locale === "zh"
  ...
  const integrationSummaryNote =
    hasRuntimeLedger
      ? [integration?.market?.note, integration?.portfolio?.note].filter(Boolean).join(" ")
      : locale === "zh"
  ...
  if (isLoading) {
    return (
      <main className="min-h-screen bg-[#F9FAFB] px-8 pb-16 pt-10">
        <div className="mx-auto max-w-[1520px] text-[#6B7280]">{t.loading}</div>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen bg-[#F9FAFB] px-8 pb-16 pt-10">
        <div className="mx-auto max-w-[1520px]">
          <Link href="/agent-arena" className="inline-flex items-center gap-2 text-[16px] text-[#8c8377]">
            <ArrowLeft className="h-4 w-4" />
            {t.back}
          </Link>
          <div className="mt-10 text-[52px] font-semibold tracking-[-0.07em] text-[#1F2937]">{t.missing}</div>
        </div>
      </main>
    );
  }
        ...
        <header className="mb-10 flex flex-wrap items-center justify-between gap-4 border-b border-[#e1d8ca] pb-5">
          <div className="flex items-center gap-5">
            <Link href="/agent-arena" className="flex items-center gap-4">
              ...
              <div>
                <div className="text-[11px] uppercase tracking-[0.22em] text-[#6B7280]">miraix</div>
        ...
        <section className="mt-8 flex flex-wrap items-center gap-8">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap items-end gap-4">
              <h1 className="text-[58px] font-semibold leading-none tracking-[-0.07em] text-[#1F2937]">
                {agent.name}
              </h1>
              <div className="pb-1 text-[18px] text-[#8c8377]">{agent.style}</div>
            </div>
          ...
          {hasRuntimeLedger ? (
            ...
            <div className="rounded-[34px] border border-[#e8e0d5] bg-white px-8 py-8 shadow-[0_24px_60px_rgba(23,29,45,0.05)]">
              ...
              <div className="mt-6 rounded-[24px] border border-[#efe7dc] bg-[#fcfaf7] px-6 py-6 text-[16px] leading-8 text-[#5f5963]">
                {locale === "zh"
                  ? "这个官方代理当前只展示两类真实信息：OKX 公共市场数据和实时 K 线。账户权益、当前持仓、执行订单、成交记录都不再展示，避免把官方策略样本误读成独立真实运行账户。"
                  : "This official agent currently shows only two real data sources: OKX public market data and live price history. Account equity, positions, orders, and fills are hidden to avoid presenting a strategy sample as a dedicated live account."}
              </div>
            </div>
          )}
        </section>
              ...
              <div className="mt-6 grid gap-6 xl:grid-cols-[0.86fr_1.14fr]">
                <div className="space-y-5">
                  ...
                  <div className="rounded-[24px] border border-[#efe7dc] bg-[#fcfaf7] p-5">
                    <div className="flex items-center justify-between gap-3">
                      <div className="text-sm text-[#9b9184]">
                        {locale === "zh" ? "模拟跟单账户" : "Demo copy accounts"}
                      </div>
                      ...
                      >
                        ...
                        {copyProfiles.length ? (
                          <option value="okx-demo-account">
                            {locale === "zh" ? "独立 OKX demo 账户" : "Dedicated OKX demo account"}
                          </option>
                        ) : null}
                      </select>
                      <button
                    ...
                    {copyMode === "okx-demo-account" ? (
                      ...
                      >
                        <option value="">
                          {locale === "zh" ? "选择一个 OKX demo follower" : "Select an OKX demo follower"}
                        </option>
                    ...
                    <div className="mt-4 space-y-2">
                      {copyTrade?.followers.filter((follower) => follower.status === "active").length ? (
                          ...
                          <div key={follower.id} className="flex items-center justify-between rounded-[16px] border border-[#efe7dc] bg-white px-4 py-3 text-sm">
                            <div>
                              ...
                              <div className="mt-1 text-xs text-[#8a8074]">
                                {locale === "zh" ? "已分配资金" : "Allocated"} {compactUsd(follower.initialCapitalUsd)} · {follower.mode === "okx-demo-account" ? follower.profileLabel || "OKX demo" : locale === "zh" ? "平台模拟" : "Platform simulation"}
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                        ...
                        <div className="rounded-[16px] border border-[#efe7dc] bg-white px-4 py-4 text-sm text-[#81786d]">
                          {locale === "zh" ? "当前没有活跃的模拟跟单账户。" : "No active demo copy accounts yet."}
                        </div>
                      )}
                    </div>
                  </div>
