...
export default function X402AutoPaymentSimple() {
...
Ready for automatic payment`);
  ...
  // Execute automatic payment using existing Solana libraries
  const executePayment = async () => {
    ...
    try {
      // Import existing Solana libraries
      ...
      // Initialize keypair from private key (base58)
      const secretKey = base58.decode(privateKey);
      const keypair = Keypair.fromSecretKey(secretKey);
      addLog(`🔑 Signer initialized: ${keypair.publicKey.toString()}`);
      
      // Connect to Solana mainnet via server proxy
      const connection = new Connection('http://localhost:3000/api/solana-rpc', 'confirmed');
      addLog('🔗 Connected to Solana mainnet (via server proxy)');
      
      // Get token info
      const usdcMint = new PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v');
      const payToAddress = new PublicKey(paymentDetails.accepts[0].payTo);
      const amount = parseInt(paymentDetails.accepts[0].amount); // Convert to number
      
      addLog(`💸 Preparing to send ${amount} USDC to ${payToAddress.toString()}`);
      ...
      addLog(`💰 Current USDC balance: ${balance.value.uiAmount || 0} USDC`);
      ...
      // For now, simulate the payment (we'll implement real signing next)
      ...
      const finalResponse = await fetch('http://localhost:3010/api/chat-new', {
        ...
        headers: {
          ...
          'X-Payment-Asset': usdcMint.toString(),
        },
        body: JSON.stringify({
          message,
          walletAddress,
...
API Response: ${finalData.message || 'Request completed'}`);
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '900px', margin: '0 auto' }}>
      <h1>🚀 X402 Auto Payment (Simplified)</h1>
