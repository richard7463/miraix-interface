...
function buildDefaultPaymentRail(preferredAsset?: string) {
  const fallbackAsset = preferredAsset === 'USDC' ? 'USDC' : 'fxUSD'
  const preferredOption =
    BASE_PAYMENT_OPTIONS.find((option) => option.assetSymbol === fallbackAsset) ||
    BASE_PAYMENT_OPTIONS[0]
...
function normalizePreviewPayload(
  ...
  const normalizedOptions = currentRail.options?.filter(
    (option): option is (typeof BASE_PAYMENT_OPTIONS)[number] =>
  ...
  const selectedAsset =
    currentRail.assetSymbol === 'USDC' || currentRail.assetSymbol === 'fxUSD'
    ...
    preview: {
      theme:
        riskMode === 'degen'
          ? 'Base Momentum Play'
          : riskMode === 'safe'
            ? 'Base Defensive Basket'
            : 'Base Balanced Rotation',
      summary: `${budgetUsd}U budget split across Base ecosystem tokens. Strategist Agent proposes the basket, Risk Agent constrains concentration and slippage, Execution Agent validates route readiness.`,
      teaser: `Three agents, one ${budgetUsd}U rotation on Base.`,
      topSymbols: ['ETH', 'cbBTC', 'USDC'],
        ...
        agentLoop: [
          {
            ...
            verdict: `Propose ${riskMode === 'safe' ? '2' : '3'}-leg basket on Base`,
            detail: `OKX Market API returns live pricing for Base ecosystem tokens. Strategist builds a ${riskMode}-mode basket weighted by 24h volume and momentum.`,
            metrics: [
          ...
          },
          {
            ...
            detail:
              'OKX DEX Aggregator returns executable swap payloads for each leg. Execution Agent confirms route availability and gas estimation on Base before unlocking.',
            metrics: [
              { label: 'Routes found', value: riskMode === 'safe' ? '2' : '3' },
              { label: 'Gas asset', value: 'ETH' },
...
export async function POST(request: NextRequest) {
  ...
  if (liveMarketPulse.length > 0) {
    ...
    fallback.preview.marketPulse = [
      { symbol: 'ETH', tag: 'ecosystem-core', weight: 0.45, priceLabel: '$2,045', change24hLabel: '-0.62%' },
      { symbol: 'cbBTC', tag: 'store-of-value', weight: 0.35, priceLabel: '$87,200', change24hLabel: '+0.88%' },
      { symbol: 'USDC', tag: 'stable', weight: 0.20, priceLabel: '$1.00', change24hLabel: '0.00%' },
    ]
  }
